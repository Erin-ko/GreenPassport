...
本章節提供適合一般使用者（非技術人員）理解的實際操作流程，使用直觀的步驟與 Emoji 呈現。

### 5.1 家庭物品的「永續生命週期管理」流程

```mermaid
graph TD
    Start(["🛒 購買新物品 <br/> (如：沙發、吸塵器、單車)"]) --> Register["📱 登入系統並登記物品<br/>(輸入名稱與材質，可按 AI 幫忙填)"]
    Register --> Use["🛋️ 日常快樂使用物品"]
    Use --> Alert{"⏰ 收到系統保養提醒？"}
    
    Alert -- 是 --> Maintain["🔧 進行保養或送修<br/>(並在系統記下一筆維護)"]
    Maintain --> Health["💚 物品健康度回滿<br/>(延壽並省下新品製造碳排)"]
    Health --> Use
    
    Alert -- 否 (想要汰換或閒置) --> Decision{"❓ 物品如何處置？"}
    
    Decision -->|還可以正常使用| Share["🤝 去『社區共享』出借或贈送"]
    Decision -->|需要尋找專業回收/轉售| Recover["♻️ 根據系統建議二手賣出或捐贈"]
    
    Share --> Reward["🏆 獲得減碳成就榮譽<br/>(在減碳報告看見自己為地球種了幾棵樹)"]
    Recover --> Reward
    Reward --> End([🌿 實踐永續綠色生活])
```
...
```mermaid
graph TD
    Start2(["⚙️ 東西少零件/需要特定工具 <br/> (例如：椅子少顆螺絲、需要電鑽)"]) --> Action{"❓ 選擇如何取得？"}
    Action -->|去大賣場買一整包| Buy["💸 花錢且造成浪費"]
    Action -->|使用社區互助| Post["📝 在『社區共享』發布募集貼文<br/>(系統會貼心幫您模糊定位保護隱私)"]
    
    Post --> Nearby["🏠 附近的鄰居看到募集資訊<br/>(發現自己手邊剛好有閒置零件)"]
    Nearby --> Chat["💬 雙方在系統內開啟『聊聊』<br/>(確認面交時間與地點)"]
    Chat --> Handover["📦 順利拿到零件/借到工具"]
    Handover --> Repair["🛠️ 修復物品/完成工作"]
    Repair --> Confirm["✅ 點擊『確認結案』<br/>(出借物品標記為已借出)"]
